...
inline static bool lflist_del_zhang(struct lflist_head *restrict head,
				    struct lflist_head *restrict target)
{
	struct lflist_head *prev;
	struct lflist_head *curr;
	...
	prev = head;
	curr_raw = ck_pr_load_ptr(&head->next);

	while (!LFLIST_END(head, UNMARK_PTR(curr_raw))) {
		curr = UNMARK_PTR(curr_raw);
		if (IS_MARKED_PTR(curr_raw)) {
			next = ck_pr_load_ptr(&curr->next);
			ck_pr_cas_ptr(&prev->next, curr_raw, next);
			curr_raw = next;
			continue;
		}
		if (curr != target) {
		...
		struct lflist_head *p_next = ck_pr_load_ptr(&prev->next);
		while (!IS_MARKED_PTR(p_next)) {
			if (ck_pr_cas_ptr(&prev->next, curr, MARK_PTR(curr)))
				break;
			p_next = ck_pr_load_ptr(&prev->next);
		}
		next = ck_pr_load_ptr(&curr->next);
		ck_pr_cas_ptr(&prev->next, MARK_PTR(curr), next);
		return true;
	}
...
static void *pthread_runner(void *arg)
{
	...
	struct integer_entry *es = malloc(sizeof(*es) * 4000);

	for (i = 1; i <= 4000; ++i) {
		es[i - 1].x = i;
		lflist_add(head, &es[i - 1].integers);
		lflist_del_zhang(head, &es[i - 1].integers);
	}
	pthread_exit(NULL);
}
